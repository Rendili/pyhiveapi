        ...
        if resp_p["original"] == "<Response [200]>":
            ...
            if "token" in info and "user" in info and "platform" in info:
                ...
                if "endpoint" in info["platform"]:
                    self.api.urls.update({"base": info["platform"]["endpoint"]})
                ...
                if "temperatureUnit" in info["user"]:
                    Data.s_temperature_unit = info["user"]["temperatureUnit"]
    ...
    def hive_api_get_weather(self):
        ...
        try:
            if Data.sess_id is not None:
                weather_url = (
                    "?postcode=" + Data.s_postcode + "&country=" + Data.s_countrycode
                )
    ...
    def initialise_api(self, username, password, interval, **kwargs):
        ...
        for product in Data.products:
            if Data.products[product]["type"] in Data.HIVE_TYPES["Sensor"]:
                ...
                try:
                    Data.NAME.update({p["id"]: p["state"]["name"]})
                    binary_sensor.append(
                        {
                            "HA_DeviceType": "binary_sensor",
                            "Hive_NodeID": p["id"],
                            "Hive_NodeName": p["state"]["name"],
                            "Hive_DeviceType": p["type"],
                        }
